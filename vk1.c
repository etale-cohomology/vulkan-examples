...
fdefe int main(){  // Pseudocode of what an application looks like. I've omitted most creation structures, almost all synchronisation and all error checking. This is not a copy-paste guide!
	...
	foru(i, 0,surfaceFormatCount)  printf("SurfaceFormat \x1b[32m%s \x1b[34m%s\x1b[0m\n", VkFormatCstr(surfaceFormats[i].format), VkColorSpaceKHRCstr(surfaceFormats[i].colorSpace));
	...
	while(vk.is_running){  // Event loop!
			...
			switch(xcb_ev->response_type & 0b01111111){
				...
				case XCB_KEY_PRESS:{  xcb_key_press_event_t* ev_key_press = (xcb_key_press_event_t*)xcb_ev;  // `xcb_generic_event_t` is a polymorphic dstruct! The first 8 bits are an index into a LUT of binary layouts!
					...
					switch(vk.keycode){
						...
						case 0x3d: break;
						// ----------------------------------------------------------------
						case 0x6f: break;  // arrow y--
						case 0x74: break;  // arrow y++
						case 0x71: break;
						case 0x72: break;

						case 0x70: break;  // page y--
						case 0x75: break;  // page y++
						case 0x6e: break;  // home
						case 0x73: break;  // end
					}
				}break;
				// ----------------------------------------------------------------------------------------------------------------------------#
		...
		vkCmdDraw(         vk.commandBuffer, 3, 1, 0, 0);  // draw k vertices
	...
	// ----------------------------------------------------------------------------------------------------------------------------#
#if 0
	// img layout transitions can be costly and require us to synchronise all access by using memory barriers when changing layouts. For example, transitioning from VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL to VK_IMAGE_LAYOUT_PRESENT_SRC_KHR requires us to make sure we are done writing all our color information before the img is moved to the present layout. We accomplish this by calling vkCmdPipelineBarrier()
	...
	}; VkDescriptorSetLayout descSetLayout; vkCreateDescriptorSetLayout(vk.device, &descSetLayoutCreateInfo, NULL, &descSetLayout);

	VkDescriptorPoolCreateInfo descPoolCreateInfo = {  // the creation info states how many descriptor sets are in this pool
	};
	VkDescriptorPool descPool;
	vkCreateDescriptorPool(vk.device, &descPoolCreateInfo, NULL, &descPool);

	VkDescriptorSetAllocateInfo descAllocInfo = {  // from pool descPool, with layout descSetLayout
	}; VkDescriptorSet descSet; vkAllocateDescriptorSets(vk.device, &descAllocInfo, &descSet);

	VkBufferCreateInfo bufferCreateInfo = {  // buffer for uniform usage, of appropriate size
	};
	VkMemoryAllocateInfo memAllocInfo = {  // skipping querying for memory requirements. Let's assume the buffer can be placed in host visible memory.
	};
	VkBuffer       buffer;  vkCreateBuffer(  vk.device, &bufferCreateInfo, NULL, &buffer);
	VkDeviceMemory memory;  vkAllocateMemory(vk.device, &memAllocInfo,     NULL, &memory);
	vkBindBufferMemory(vk.device, buffer, memory, 0);

	void* data; vkMapMemory(vk.device, memory, 0, VK_WHOLE_SIZE, 0, &data);
	...
	// ----------------------------------------------------------------------------------------------------------------------------#
	// render
	VkSubmitInfo submitInfo = {  // this contains a reference to the above commandBuffer to submit
		sType:              VK_STRUCTURE_TYPE_SUBMIT_INFO,
		commandBufferCount: 1,
		pCommandBuffers:    &commandBuffer,
	}; vkQueueSubmit(vk.queue, 1, &submitInfo, NULL);  // submit the cmd buf to the queue

	VkPresentInfoKHR presentInfo = {  // vk.swapchain and currentSwapImage are used here
		stype:              VK_STRUCTURE_TYPE_PRESENT_INFO_KHR,                 // VkStructureType          sType;
		waitSemaphoreCount: 1,                                                  // uint32_t                 waitSemaphoreCount;
		pWaitSemaphores:    &vk.per_frame[index].swapchain_release_semaphore,  // const VkSemaphore*       pWaitSemaphores;
		swapchainCount:     1,                                                  // uint32_t                 swapchainCount;
		pSwapchains:        &vk.swapchain,                                     // const VkSwapchainKHR*    pSwapchains;
		pImageIndices:      &index,                                             // const uint32_t*          pImageIndices;
		pResults:           NULL,                                               // VkResult*                pResults;
	}; vkQueuePresentKHR(vk.queue, &presentInfo);  // present the swap chain img
#endif
